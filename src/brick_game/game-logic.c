...
/// Обрабатка ввода пользователя и выполнение соответствующего действия
...
// Создание новой фигуру и помещение ее в игровое поле
...
// Основная функция расчета состояния игры
void calculate(Game *tetg) {
  if (tetg->ticks_left <= 0 && tetg->state != PAUSE && tetg->state != INIT)
    calcOne(tetg);  // Замедление игры до 30 кадров в секунду
...
// Обрабатка одного тика игры, перемещение фигуры вниз
...
// Перемещение фигуры на одну позицию
...
// Проверка на наличие столкновений фигуры с границами поля или другими блоками
...
// Удаление заполненных линий и возврат их количества
...
// Сдвиг линии вниз на одну позицию
...
// Обрабатка вращения фигуры с проверкой на столкновения
...
// Устанавка фигуры на игровое поле
...
// Подсчет очков в зависимости от удаленных линий
void countScore(Game *tetg) {
  ...
  if (tetg->score > tetg->high_score) { // Обновление рекорда, если текущий счет превышает его
    tetg->high_score = tetg->score;
    saveHighScore(tetg->high_score);
  }

// Обновление уровня и скорости игры в зависимости от набранных очков
