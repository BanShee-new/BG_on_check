...
Game *createGame(int field_width, int field_height, int figures_size,
  ...
  tetg->score = 0; // Начальный счет
  tetg->high_score = loadHighScore(); // Загрузка рекорда
  tetg->ticks = 30; // Установка начального количества тиков
  tetg->ticks_left = 30; // Установка оставшихся тиков
  tetg->speed = 1; // Установка начальной скорости
  tetg->level = 1; // Установка начального уровня

  tetg->pause = 1; // Установка состояния паузы
  tetg->state = INIT; // Установка начального состояния игры

  tetg->next = rand() % tetg->figurest->count; // Генерация следующей фигуры случайным образом
...
// Создание поля для отображения игры
...
// Создание поля для следующей фигуры
